...
import be.sgerard.i18n.model.workspace.dto.ReviewDto;
...
public class WorkspaceDtoEnricher {
    ...
    /**
     * Maps the {@link AbstractReviewEntity review entity} to its {@link ReviewDto DTO}.
     */
    private ReviewDto mapReview(AbstractReviewEntity review) {
        if(review instanceof GitHubReviewEntity){
            final GitHubReviewEntity gitHubReview = (GitHubReviewEntity) review;

            return GitHubReviewDto.builder()
                    .pullRequestBranch(gitHubReview.getPullRequestBranch())
                    .pullRequestNumber(gitHubReview.getPullRequestNumber())
                    .build();
        } else {
            throw new UnsupportedOperationException("The entity [" + review + "] is not supported.");
        }
    }
}
