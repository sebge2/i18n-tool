...
export class SnapshotService {
    ...
    constructor(private apiSnapshotService: ApiSnapshotService,
        ...
        this._snapshots$ = this._synchronizedSnapshots
            .collection
    ...
    public createSnapshot(comment?: string, encryptionPassword?: string): Observable<Snapshot> {
        return this.apiSnapshotService
            .create1({comment: comment, encryptionPassword: encryptionPassword})
            .pipe(
    ...
    public importSnapshot(file: Blob, encryptionPassword?: string): Observable<Snapshot> {
        return this.apiSnapshotService
            .importZipForm(encryptionPassword, file)
            .pipe(
                map(dto => Snapshot.fromDto(dto)),
                tap(snapshot => this._synchronizedSnapshots.add(snapshot)),
            );
