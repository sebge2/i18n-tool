...
export class ScheduledTaskService {
  private readonly _synchronizedTaskDefinitions: SynchronizedCollection<
    ScheduledTaskDefinitionDto,
    ScheduledTaskDefinition
  >;
  private readonly _taskDefinitions$: Observable<ScheduledTaskDefinition[]>;

  private readonly _synchronizedTaskExecutions: SynchronizedCollection<
    ScheduledTaskExecutionDto,
  ...
  constructor(
    private apiScheduledTaskService: ApiScheduledTaskService,
    ...
    this._taskDefinitions$ = this._synchronizedTaskDefinitions.collection.pipe(
      catchError((reason) => {
    ...
    this._taskExecutions$ = this._synchronizedTaskExecutions.collection.pipe(
      catchError((reason) => {
  ...
  public getTaskDefinition(definitionId: string): Observable<ScheduledTaskDefinition | undefined> {
    return this.getTaskDefinitions().pipe(
  ...
  public enable(taskDefinition: ScheduledTaskDefinition): Observable<ScheduledTaskDefinition> {
    return this.apiScheduledTaskService.updateStatus(taskDefinition.id, 'ENABLE').pipe(
  ...
  public disable(taskDefinition: ScheduledTaskDefinition): Observable<ScheduledTaskDefinition> {
    return this.apiScheduledTaskService.updateStatus(taskDefinition.id, 'DISABLE').pipe(
      map((dto) => ScheduledTaskDefinition.fromDto(dto)),
      tap((snapshot) => this._synchronizedTaskDefinitions.update(snapshot))
    );
  }
