...
import { PlayButtonState } from '@i18n-core-shared';
...
export class ScheduledTaskDefinitionListComponent implements OnInit, OnDestroy {
  ...
  constructor(public scheduledTaskService: ScheduledTaskService, private notificationService: NotificationService) {}
  ...
  public getPlayState(definition: ScheduledTaskDefinition): PlayButtonState {
    return definition.enabled ? PlayButtonState.STARTED : PlayButtonState.STOPPED;
  }
  ...
  public onChangeStatus(definition: ScheduledTaskDefinition) {
    ...
    obs
      .toPromise()
      .catch((error) =>
        this.notificationService.displayErrorMessage('ADMIN.SCHEDULED_TASKS.ERROR.SCHEDULE_UNSCHEDULE', error)
      )
