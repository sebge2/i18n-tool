...
import {PreferencesService} from "../../core/shared/service/preferences.service";
...
export class DictionaryService {
    ...
    constructor(private _apiDictionaryService: ApiDictionaryService,
        ...
        this._entries$ = this._synchronizedEntries
            .collection
        ...
        this._locales$ = combineLatest([this._localeService.getAvailableLocales(), this._localeService.getDefaultLocales(), this._preferredLocales$])
            .pipe(map(([allLocales, defaultLocales, preferredLocales]) => {
    ...
    public getDictionary$(): Observable<DictionaryEntry[]> {
        return this._entries$;
    }

    public createTranslation(request: DictionaryEntryCreationDto): Observable<DictionaryEntry> {
        return this._apiDictionaryService
            .create3(request)
            .pipe(map((dto: DictionaryEntryDto) => DictionaryEntry.fromDto(dto)));
    }

    public updateTranslations(patches: DictionaryEntryPatchDto[]): Observable<DictionaryEntry[]> {
        return this._apiDictionaryService
            .updateEntries(patches)
            .pipe(map((entries: DictionaryEntryDto[]) => _.map(entries, entry => DictionaryEntry.fromDto(entry))));
    }

    public delete(entryId: string): Observable<any> {
    ...
    public upload(file: ImportedFile): Observable<DictionaryEntry[]> {
        return this._apiDictionaryService.importDictionary(file.file, 'import')
            .pipe(map((entries: DictionaryEntryDto[]) => _.map(entries, entry => DictionaryEntry.fromDto(entry))));
    }
    ...
    public getLocales$(): Observable<TranslationLocale[]> {
        return this._locales$;
    }

    public setPreferredLocales(preferredLocales: TranslationLocale[]): Observable<TranslationLocale[]> {
