...
export class DictionarySettingsDialogComponent implements OnInit, OnDestroy {
  ...
  constructor(
    private _dialogRef: MatDialogRef<DictionarySettingsDialogComponent>,
  ...
  public onSelect(event: { fromIndex: number; toIndex: number }): void {
    transferArrayItem(this.availableLocales, this.selectedLocales, event.fromIndex, event.toIndex);
  }

  public onUnselect(event: { fromIndex: number; toIndex: number }) {
    transferArrayItem(this.selectedLocales, this.availableLocales, event.fromIndex, event.toIndex);
  }
