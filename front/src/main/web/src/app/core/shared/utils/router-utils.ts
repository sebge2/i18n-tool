...
export function getRouteParamsCollection<T extends { [key: string]: any }, K extends keyof T>(
  paramName: string,
  params: Params,
  availableValues: T[],
  defaultValues: T[],
...
export function getRouteParamSimpleValue<T>(
  paramName: string,
  params: Params,
  defaultValue: T,
...
export function getRouteParamObject<T extends { [key: string]: any }, K extends keyof T>(
  paramName: string,
  params: Params,
  availableValues: T[],
  defaultValue: T,
  ...
  if (_.some(routeParams)) {
    ...
    return _.some(filteredElements) ? filteredElements[0] : null;
  } else {
    return defaultValue;
  }
}

export function getRouteParamEnum<T extends { [key: number]: string | number }>(
  paramName: string,
  type: T,
