...
export class TreeNode {
    ...
    constructor(public data: TreeObject,
                public level: number,
...
export class TreeDataSource implements DataSource<TreeNode> {
    ...
    constructor(private _treeControl: TreeControl<any>,
                private _source: TreeObjectDataSource) {
    }
    ...
    connect(collectionViewer: CollectionViewer): Observable<TreeNode[] | ReadonlyArray<TreeNode>> {
        ...
        return merge(collectionViewer.viewChange, this._treeNodes$)
            .pipe(
    ...
    public load(): void {
        this._source
            .getRootObjects()
            .pipe(takeUntil(this._destroyed$))
            .subscribe((rootNodes: TreeObject[]) =>
                this.treeNodes = rootNodes.map(rootNode => new TreeNode(rootNode, 1))
            );
    }
