...
export class TranslationLocaleService {
  ...
  constructor(
    private apiService: ApiTranslationLocaleService,
    ...
    this._locales$ = this._synchronizedLocales$.collection.pipe(
      catchError((reason) => {
    ...
    this._preferredLocales$ = combineLatest([
    ...
    this._autoDetectedLocales$ = this.getAvailableLocales().pipe(
    ...
    this._defaultLocales$ = combineLatest([this.getPreferredLocales(), this.getAutoDetectedLocales()]).pipe(
  ...
  public createLocale(creationTranslationLocale: TranslationLocaleCreationDto): Observable<TranslationLocale> {
    return this.apiService.create(creationTranslationLocale).pipe(
  ...
  public updateLocale(translationLocale: TranslationLocale): Observable<TranslationLocale> {
    return this.apiService.update(translationLocale.toDto(), translationLocale.id).pipe(
      map((dto) => TranslationLocale.fromDto(dto)),
  ...
  public deleteLocale(translationLocale: TranslationLocale): Observable<TranslationLocale> {
    return this.apiService.delete1(translationLocale.id).pipe(
  ...
  private static findMatchingLocale(browserLocale: Locale, translationLocales: TranslationLocale[]): TranslationLocale {
    return _.find(translationLocales, (translationLocale) =>
      TranslationLocaleService.isMatchingBrowserLocale(translationLocale, browserLocale)
    );
  }
