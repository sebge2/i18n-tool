...
import {TextTranslations} from "../model/text/text-translations.model";
...
export class TranslationService {
    ...
    translateText(text: string, fromLocaleId: string, targetLocaleId: string): Observable<TextTranslations> {
        return this._translationService
            .translateText(
                {
                    text: text,
                    fromLocaleId: fromLocaleId,
                    targetLocaleId: targetLocaleId
                },
                'translateText'
            )
            .pipe(map(response => TextTranslations.fromDto(response)));
    }
}
