...
export class WorkspaceService {
  ...
  constructor(
    private apiWorkspaceService: ApiWorkspaceService,
    ...
    this._workspaces$ = this._synchronizedWorkspaces$.collection.pipe(
      catchError((reason) => {
  ...
  getRepositoryWorkspaces(repositoryId: string): Observable<Workspace[]> {
    return this.getWorkspaces().pipe(
      map((workspaces) => workspaces.filter((workspace) => _.isEqual(workspace.repositoryId, repositoryId)))
    );
  }

  getWorkspace(workspaceId: string): Observable<Workspace | undefined> {
    return this.getWorkspaces().pipe(
  ...
  getWorkspaceBundleFiles(workspaceId: string): Observable<BundleFile[] | undefined> {
    if (!this._cachedWorkspaceBundleFiles.has(workspaceId)) {
      this._cachedWorkspaceBundleFiles.set(
        workspaceId,
        this.getWorkspace(workspaceId).pipe(
          distinctUntilKeyChanged('lastSynchronization'),
          mergeMap((workspace) =>
            workspace ? this.apiWorkspaceService.findWorkspaceBundleFiles(workspaceId) : of(undefined)
          ),
  ...
  getWorkspaceBundleFile(workspaceId: string, bundleFiledId: string): Observable<BundleFile | undefined> {
    return this.getWorkspaceBundleFiles(workspaceId).pipe(
      map((bundleFiles) => _.find(bundleFiles, (bundleFile) => _.eq(bundleFile.id, bundleFiledId)))
    );
  }

  initialize(workspaceId: string): Observable<Workspace> {
    return this.apiWorkspaceService.executeAction(workspaceId, '', 'INITIALIZE').pipe(
      map((workspace) => Workspace.fromDto(workspace)),
      tap((workspace) => this._synchronizedWorkspaces$.update(workspace))
    );
  }

  synchronize(repositoryId: string): Observable<Workspace[]> {
    return this.apiWorkspaceService.synchronize(repositoryId, 'SYNCHRONIZE').pipe(
  ...
  synchronizeWorkspace(workspace: Workspace): Observable<Workspace> {
    return this.apiWorkspaceService.executeAction(workspace.id, '', 'SYNCHRONIZE').pipe(
      map((workspace) => Workspace.fromDto(workspace)),
      tap((workspace) => this._synchronizedWorkspaces$.update(workspace))
    );
  }

  publishAll(workspaces: string[], comment: string): Observable<Workspace[]> {
    return this.apiWorkspaceService
      .publish({ workspaces: workspaces, message: comment }, 'PUBLISH')
      .pipe(map((workspaces) => _.map(workspaces, (workspace) => Workspace.fromDto(workspace))));
  }
}
